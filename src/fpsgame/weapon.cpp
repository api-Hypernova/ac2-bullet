...
//                regular_particle_splash(PART_BLOOD, 5, 300, pos, 0x99FFFF, .6f, 1, 0, 0); // 1000, 100);
            ...
            if(bnc.bouncetype==BNC_ORB||bnc.bouncetype==BNC_PROP)
            {
                ...
                if(bnc.o.dist(player1->o)<16 && bnc.bouncetype==BNC_PROP && bnc.vel.magnitude()<80)
                {
                    ...
                    bnc.vel.z=15.f; //even though they float upwards :)
                    }
                }
                loopv(bouncers)
                {
...
//                }
    ...
    void gibeffect(int damage, const vec &vel, fpsent *d)
    {
        ...
        else if(damage<500 && guns[d->diedgun].splash>30)
        {
            if(blood) loopi(min(gibnum, 50)+1) spawnbouncer(from, vel, d, BNC_GIBS, damage/4);
            if(blood) particle_splash(PART_BLOOD, damage*3, 10000, d->o, 0x99FFFF, 2.f, 4000); //damage/5
            playsound (S_GIB, &d->o);
        }
        ...
        if(d->gunselect!=GUN_TELEKENESIS&&d->gunselect!=GUN_TELEKENESIS2&&d->gunselect!=GUN_FIST)spawnbouncer(from, vel, d, BNC_WEAPON, damage/3);
    }
...
//                    }
    ...
    void radialeffect(dynent *o, const vec &v, int qdam, fpsent *at, int gun) //v=point of explosion
    {
        ...
        if(dist<guns[gun].splash)
        {
            ...
            if(blood && qdam<800 && o->type==ENT_PLAYER && damage) { particle_splash(PART_BLOOD, 1, 150, o->o, 0x99FFFF, 6.f, 50, 0, NULL, 2, false, 3);
                adddecal(DECAL_BLOOD, o->o, vec(v).sub(o->o).normalize(), rnd(15)+10.f, bvec(0x99, 0xFF, 0xFF));
                particle_splash(PART_SPARK, 100, 300, o->o, 0xFF0000, 0.1f, 250);
            }
            //if(at==player1)conoutf(CON_TEAMCHAT, "-%d", damage*(player1->quadmillis?(int)1.5f:1));
            ...
            for(;;)
            {
                ...
                loop(r, guns[d->gunselect].rays) if(!done[r] && (cl = intersectclosest(from, rays[r], d, dist)))
                {
                    if(!o || o==cl)
                    {
                        ...
                        blooddest.z += rnd(2)+2;
                        if(blood && (cl->type==ENT_PLAYER || cl->type==ENT_AI) && r%2==0)adddecal(DECAL_BLOOD, blooddest, vec(from).sub(blooddest).normalize(), rnd(5)+13.f, bvec(0x99, 0xFF, 0xFF)); //don't do blood for all bbs that hit
                        shorten(from, rays[r], dist);
                        if(blood && qdam<500 && qdam>0 && (cl->type==ENT_PLAYER || cl->type==ENT_AI)){
                                                        particle_splash(PART_BLOOD, 1, 150, rays[r], 0x99FFFF, 6.f, 50, 0, NULL, 2, false, 3); particle_splash(PART_SPARK, 100, 300, rays[r], 0xFF0000, 0.1f, 230);
                        }
                ...
                //if(d==player1 && hitrays && cl->type!=ENT_INANIMATE)conoutf(CON_TEAMCHAT, "-%d", (int)hitrays*(guns[d->gunselect].damage)*(d->headshot?3:1)*(d->quadmillis?1.5f:1));
                if(!raysleft) break;
            }
...
//        }
        if((o = intersectclosest(from, to, d, dist)) && guns[d->gunselect].rays==1)
        {
            vec blooddest=to;
            blooddest.z+=rnd(2)+2;
            if(blood && d->gunselect!=GUN_TELEKENESIS2  && (o->type==ENT_PLAYER || o->type==ENT_AI))adddecal(DECAL_BLOOD, blooddest, vec(from).sub(blooddest).normalize(), rnd(5)+13.f, bvec(0x99, 0xFF, 0xFF));
            shorten(from, to, dist);
            if(blood && qdam<500 && d->gunselect!=GUN_TELEKENESIS2 && (o->type==ENT_PLAYER || o->type==ENT_AI)){particle_splash(PART_BLOOD, 1, 150, to, 0x99FFFF, 6.f, 50, 0, NULL, 2, false, 3); particle_splash(PART_SPARK, 100, 300, to, 0xFF0000, 0.1f, 230);
            }
            hitpush(qdam, o, d, from, to, d->gunselect, 1);
            if(isheadshot(o, to)&& (d==player1 || d->ai)) { qdam *= 3; d->headshots=1; }
            //if(d==player1 && qdam>0 && o->type!=ENT_INANIMATE)conoutf(CON_TEAMCHAT, "-%d", (int)(guns[d->gunselect].damage)*(d->headshot?3:1 )*(d->quadmillis?1.5f:1));
        }
...
//                }
                ...
                if(d->gunselect!=GUN_MAGNUM && d->gunselect!=GUN_ELECTRO && d==hudplayer()) {d->pitch+=-1+rnd(3);
                d->yaw+=-1+rnd(3);
                }
...
//        }
    ...
    static const char * const weaponmodelnames[NUMGUNS] = { "", "items/cartridges", "items/bullets", "items/rockets", "items/rrounds" "pickups/hand_grenade", "pickups/electro", "pickups/electro", "items/bullets", "items/cartridges", "Hypercubehudguns/pyccna_m4a1", "Hypercubehudguns/pyccna_m4a1", "", "", "Hypercubehudguns/pyccna_fn57" };
        //enum { GUN_FIST = 0, GUN_SG, GUN_CG, GUN_RL, GUN_MAGNUM, GUN_HANDGRENADE, GUN_ELECTRO, GUN_ELECTRO2, GUN_CG2, GUN_SHOTGUN2, GUN_SMG, GUN_SMG2, GUN_TELEKENESIS, GUN_TELEKENESIS2, GUN_PISTOL, GUN_FIREBALL, GUN_ICEBALL, GUN_SLIMEBALL, GUN_BITE, GUN_BARREL, NUMGUNS };
