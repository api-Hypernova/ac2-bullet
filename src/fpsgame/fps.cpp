...
namespace game
{
    ...
    void updateworld()        // main game update loop
    {
        ...
        if(lastmillis-player1->uncrouchtime<10 && player1->timeinair<300)entinmap(player1);
    ...
    void deathstate(fpsent *d, bool restore)
    {
        ...
        if(d==player1)
        {
            ...
            if(lookupmaterial(camera1->o)!=MAT_WATER)
            {
               ...
               if (d->playermodel==1 || d->playermodel==3 || d->playermodel==4) playsound(S_DEATHE); //snout
            }
        }
        else
        {
            ...
            if(lookupmaterial(camera1->o)!=MAT_WATER && lookupmaterial(d->o)!=MAT_WATER)
            {
               ...
               // if(d->playermodel==0 || d->playermodel==2) playsound(S_DEATHR, &d->o); //fixit
                if (d->playermodel==1 || d->playermodel==3 || d->playermodel==4) playsound(S_DEATHE, &d->o); //snout
            }
        }
    }
