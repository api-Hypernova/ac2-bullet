...
struct physent                                  // base entity type, can be affected by physics
{
    ...
    bool blocked;                               // used by physics to signal ai
    btRigidBody *body; //this will be our interface from cube to bullet

//in the consturctor set the weight to 80 or something.
       physent() : o(0, 0, 0), deltapos(0, 0, 0), newpos(0, 0, 0), yaw(0), pitch(0), roll(0), maxspeed(100),
                  radius(4.1f), eyeheight(18), maxheight(18), aboveeye(2), xradius(4.1f), yradius(4.1f), zmargin(0), weight(80),
                  state(CS_ALIVE), editstate(CS_ALIVE), type(ENT_PLAYER),
                  collidetype(COLLIDE_ELLIPSE),
                  blocked(false)
                  { reset(); }
