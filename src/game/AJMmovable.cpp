...
namespace game
{
	...
	struct bulletmovable {
		...
		bulletmovable(const entity &e) : 
			...
			yaw(e.attr2+90.f), pitch(e.attr3), roll(e.attr4), 
    ...
    void clearbulletmovables()
    {
        ...
        loopv(entities::ents) 
        {
			...
			if (e.type == MAPMODEL){
				const char *mdlname = mapmodelname(e.attr1);
                setcbfrommodel(vec(e.o), mdlname, vec(e.attr2 - 90.f, e.attr3, e.attr4), 0);
			}
	...
	void QuaternionToEuler(const btQuaternion &TQuat, vec &TEuler) {
		...
		float YSquared = Y * Y;
		float ZSquared = Z * Z;
		float hey = (-2.0f * ((X * Z) - (Y * W)));
		if (hey >= 1.0f) hey = 1.00f; else if (hey <= -1.0f) hey = -1.00f;
	...
	void updatebulletmovables(vec tmpvec, vec tmprot, int j)
	{
		loopv(bulletmovables){
			...
			if (!tmpvec.iszero()) m->o = vec(tmpvec.x * 17, tmpvec.z * 17, tmpvec.y * 17);
            m->yaw = tmprot.z;
    ...
    void renderbulletmovables()
    {
        loopv(bulletmovables)
        {
            ...
            pos.mul(-m.radius.z);
            pos.add(m.o); //some not supposed to commented out?
			//particle_flare(pos.add(m.o),m.o , 0, PART_STREAK, 0x0000FF);

			const char *mdlname = mapmodelname(m.mapmodel);
            if(!mdlname) continue;
            rendermodel(mdlname, ANIM_MAPMODEL|ANIM_LOOP, pos, m.yaw, m.pitch, m.roll, MDL_CULL_VFC | MDL_CULL_DIST|MDL_CULL_OCCLUDED|MDL_CENTER_ALIGNED );
